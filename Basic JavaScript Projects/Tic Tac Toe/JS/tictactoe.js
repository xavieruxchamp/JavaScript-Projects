...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if (activePlayer === 'X') {
            ...
            //Active player may only be 'X' or 'O', if n ot 'X' it must be 'O'
        } else {
        ...
        audio('./media/zapsplat_cartoon_anime_low_pitched_warble_92489.mp3');
    ...
    function computersTurn() {
        ...
        let success = false;
        //This condition stores a random number 0-8.
        let pickASquare;
...
function checkWinConditions() {
    ...
    //This function checks is an array includes 3 strings. If is used to check for
    //each win condition.
...
function drawWinLine(coordXl, coordYl, coordX2, coordY2) {
    ...
    audio('./media/zapsplat_emergency_police_siren_isolated_clean_short_warning_burst_attention_63711.mp3');
...
function resetGame() {
    //This for loop will iterate through each HTML square element.
    for (let i = 0; i < 9; i++) {
    ...
    //This removes our array so it is empty and we can start over.
    selectedSquares = [];
}